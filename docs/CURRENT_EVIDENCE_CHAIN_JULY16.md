...
| Transition-label robustness | SAILOR + SRD | Across `576` threshold settings, SAILOR retained higher distant-growth and mixed-growth/loss fractions than SRD with positive gap rate `1.00`; default hard regions were spread across many SAILOR patients | The central SAILOR-vs-SRD transition gap is not just a single-threshold artifact | Strong robustness evidence | Core-loss and loss-dominant gaps are less stable and should remain secondary |

## Ruthless Claim-Evidence Audit

This section is intentionally stricter than the compact evidence table above. A result can be interesting without being ready to carry a central claim. The goal is to prevent the project from drifting into polished but fragile conclusions.

| Proposed Claim | Current Verdict | What Supports It | What Could Break It | Required Before It Becomes Central |
|---|---|---|---|---|
| Short-horizon tumor forecasting is not a uniform task | Strong descriptive claim | SRD tier/horizon differences; SAILOR LOCF operating range; SAILOR transition taxonomy showing mixed growth/loss and non-boundary change | If effects disappear under patient-level grouping or if categories are mostly threshold artifacts | Patient-count reporting, split-level summaries, and clear threshold/radius sensitivity |
| LOCF is strong because many short-horizon transitions preserve substantial tumor mass | Strong but should be phrased carefully | LOCF operating-range analysis; high Dice in low-burden/persistence-friendly cases; SRD Tier A behavior | LOCF Dice is mathematically coupled to change burden, so this can become tautological if written carelessly | Phrase as an operating-range characterization, not a surprising causal discovery |
| Calendar time alone is insufficient to define short-term difficulty | Strong analytical claim | SAILOR operating-range analysis shows transition burden matters more than raw `delta_days` | If interval bins are too sparse or confounded by patient/split | Report both calendar interval and observed transition burden; avoid saying calendar time is irrelevant |
| SRD is useful for isolating growth, shrinkage, persistence, tier, and horizon effects | Strong within-SRD claim | SRD taxonomy shows clean separation across Tier A/B/C, horizons, net-growth/net-shrinkage | Single SRD realization may make exact proportions fragile | Treat exact numbers as dataset-specific; use SRD for mechanism isolation, not population prevalence |
| SRD is a realistic surrogate for SAILOR | Reject | Domain-gap analysis directly argues against this | SAILOR has transition patterns SRD lacks: mixed growth/loss, scale, high-change tails, distant growth | Do not make this claim |
| The SRD-SAILOR domain gap is scientifically useful | Strong framing claim | Domain-gap and threshold-robustness analyses quantify what controlled SRD omits and what real SAILOR adds: mixed growth/loss, distant growth, high-change tails, and scale | Reviewers may read mismatch as generator weakness unless the role distinction is explicit | Put the role distinction early: SRD = controlled mechanism isolation; SAILOR = real transition complexity; keep core-loss/loss-dominant evidence secondary |
| Future-growth localization should be evaluated separately from full-mask Dice | Strong conceptual claim | Ranking metrics reveal useful spatial signal even when Dice gains are modest; ranking-Dice tradeoff observed | If ranking baselines explain most gains or ranking does not improve downstream masks | Always compare against distance/random baselines and budgeted-mask conversion |
| Boundary proximity is a strong growth-front prior | Strong descriptive prior | Distance baseline and delayed-hit analyses on SRD/SAILOR; biological plausibility | Some SAILOR growth is non-boundary/distant; radius choice affects magnitude | Report as a strong but incomplete prior, not a sufficient forecaster; distant-growth robustness makes this caution central |
| Image/context models add useful signal beyond distance | Moderate, promising | Cropped SAILOR ranking: hybrid AP above distance; paired gains in most samples | Small SAILOR sample count; potential crop/selection effects; checkpoints not extensively replicated | More patient-level validation, repeated split/seed, and direct comparison to simple morphology/time features |
| Growth-budget estimation is the key bottleneck | Strong methodological diagnosis | Ranking can be good while Dice suffers; budgeted policies improve SRD; SAILOR growth-only gains are small and conservative | If better architectures solve budget implicitly, this may become less central | Test simple budget predictors using only forecast-origin features |
| Growth-only residual forecasting is a credible method direction | Moderate, not central yet | Seed 42/123 SAILOR growth-only runs select same conservative budget and show small positive gaps | Gains are tiny on test; few patients; shrinkage unresolved; could be threshold/budget tuning | More splits/seeds, patient bootstrap, fair same-resolution direct baselines, and explicit failure analysis |
| The current method already beats standard forecasting baselines in a meaningful way | Weak / not ready | Some positive growth-only and hybrid signals | Effect sizes are small; test uncertainty high; direct ResUNet/LOCF comparisons remain difficult | Do not center the project on this claim yet |
| Loss/shrinkage can be predicted symmetrically with growth | Reject for now | Residual-change experiments struggled with loss; growth-only framing performed cleaner | Loss may reflect treatment, registration, edema/segmentation variation, or true biology | Treat loss as separate uncertainty-sensitive component, not symmetric residual target |
| A regime-aware prior can improve forecasting | Plausible but unproven | Evidence supports decomposition and candidate priors; method probes show direction | No mature architecture or robust performance result yet | Develop method only after budget/predictability analysis clarifies what can be known at forecast origin |
