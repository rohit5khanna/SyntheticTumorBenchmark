...
- `scripts/build_main_evidence_package.py` has been added to curate manuscript-facing tables and figures from the completed output folders.

### Missing
- Run the main evidence package builder on the current Drive outputs.
- Inspect whether each generated artifact is main-text, supporting, diagnostic, or supplement-only.
...
| SAILOR exposes real transition complexity absent from SRD | SAILOR taxonomy, radius sensitivity, domain-gap analysis, threshold robustness, patient-spread audit | Strong descriptive claim | Compact final table/figure package | Core-loss/loss-dominant descriptors are less stable and should remain secondary |
...
| Forecast-origin features can estimate useful correction budgets | SAILOR fixed-split budget audit and 100 repeated patient-split budget stability audit; history-only ridge mean predicted-direction oracle gap +0.142, q25 +0.125, positive in all repeats | Strong calibration-feasibility premise | No-oracle spatial model; external validation; check sensitivity to train fraction and forecast window construction | Current result still assumes perfect spatial localization and should not be read as final mask-forecasting performance |
| A distance-to-current-mask spatial prior is enough to convert predicted correction budgets into better masks | SAILOR budgeted distance forecast on split v2; predicted-budget distance policies fall below LOCF on validation/test; true-budget distance gives only small gains | Rejected as a sufficient method | Keep only as a negative bottleneck result; compare against learned spatial field | If ignored, the method story could look like threshold engineering rather than a real diagnosis of spatial localization |
| The current growth-only learned probability field is sufficient for budgeted LOCF correction | SAILOR budgeted learned-field result: probabilities separate true growth from non-growth, but predicted-budget edits reduce Dice on validation/test; true-budget policies show only limited headroom | Rejected as a sufficient current method, but useful diagnostic evidence | Failure diagnosis: budget ratio, direction leakage, fragmentation, distance to true growth, Dice gain/cost | Without the diagnosis, this could be mistaken as a generic model failure rather than a specific mask-editing bottleneck |
| Conservative gating/scaling can make learned growth-field correction safer than unrestricted top-k edits | SAILOR seed-42 and seed-123 conservative policies both select gate 0.5 and budget scale 0.1 on validation; validation gaps remain small positive; seed-42 test gap is +0.0017 but seed-123 test gap is -0.0002; patient-bootstrap CIs cross zero for overall test gaps; net-growth signal is positive in seed 42 but not stable in seed 123; shrinkage remains harmful | Diagnostic/bottleneck evidence, not a robust method claim | Do not tune further unless redesigning direction/loss handling; use as evidence for over-editing, shrinkage leakage, and patient-level instability | Overclaiming this would weaken the research; its value is in explaining why naive learned-field correction is not enough |
...
6. Treat the conservative-policy seed-robustness audit as completed: validation selection is stable, but patient-bootstrap test evidence does not support a robust method improvement. Use this as bottleneck evidence, not as a central forecasting result.
...
21. Run `scripts/build_main_evidence_package.py` to export the current manuscript-facing table/figure package before writing.
