...
| Forecast-origin features can estimate useful correction budgets | SAILOR budget-predictability audit; predicted-direction budget oracle gaps about +0.18 Dice on val/test under perfect localization | Promising method-design premise | Repeated patient splits; separate history/no-interval from time/treatment shortcuts; no-oracle spatial model | Current result still assumes perfect spatial localization and may include operating-context confounding |
...
15. Stress-test budget predictability with repeated patient splits before treating it as a method ingredient. The current fixed split suggests useful calibration signal, but time/treatment-only feature sets are suspiciously strong on test.
