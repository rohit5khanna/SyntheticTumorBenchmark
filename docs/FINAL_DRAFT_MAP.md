...
| Learned growth probability can act as the spatial field for budgeted LOCF correction | New budgeted learned-growth-field evaluator added; result pending | Open, high-priority method test | Run with growth-only ResUNet checkpoint at matching stride; compare predicted-budget and true-budget policies | If it fails, the current method direction needs stronger spatial modeling, not more budget tuning |
...
17. Combine forecast-origin budget calibration with learned spatial score fields. Candidate spatial sources: growth-only ResUNet probability, cropped growth-ranking score, or a lightweight growth-front CNN.
