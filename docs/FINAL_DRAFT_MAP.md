...
| Conservative gating/scaling can make learned growth-field correction safer than unrestricted top-k edits | SAILOR seed-42 conservative policy: validation-selected gate 0.5 and budget scale 0.1; validation gap +0.005, test gap +0.002 versus LOCF; net-growth gains with small shrinkage harm | Weak positive, exploratory method signal | Repeat with seed-123 checkpoint; patient bootstrap; compare at matching resolution to LOCF and growth-only selected policy | Small gains could disappear under seed/split changes and should not be overclaimed |
