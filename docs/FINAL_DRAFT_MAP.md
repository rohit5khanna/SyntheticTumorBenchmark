...
| Conservative gating/scaling can make learned growth-field correction safer than unrestricted top-k edits | SAILOR seed-42 and seed-123 conservative policies both select gate 0.5 and budget scale 0.1 on validation; validation gaps remain small positive; seed-42 test gap is +0.002 but seed-123 test gap is approximately 0/slightly negative; net-growth cases benefit more than shrinkage cases | Diagnostic/exploratory, not yet a robust method claim | Aggregate seed-level table, patient bootstrap, additional seeds or repeated patient splits, and same-resolution comparisons | Small gains are not stable enough for a central performance claim; strongest current use is as evidence for over-editing and shrinkage leakage |
...
18. Treat the conservative gate/scale policy ablation as a diagnostic result, not a finished method. The seed-42 and seed-123 runs support the over-editing/shrinkage-leakage diagnosis, but the held-out test gain is not stable enough to claim robust improvement over LOCF.
