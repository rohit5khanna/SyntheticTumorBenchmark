...
## 2026-07-20 - SAILOR Spatial Transition Taxonomy Result

- Ran the spatial transition-taxonomy analysis on the SAILOR input-length-3, horizon-1 manifest across train/validation/test.
- Overall SAILOR transition set: 136 transitions from 23 patients, mean LOCF Dice 0.639, median LOCF Dice 0.690, mean delta 74.5 days.
- Decomposition showed substantial observed transition burden even at one-session horizon: mean relative new growth 1.092, mean relative apparent loss 0.285, and mean relative absolute change 1.376 relative to input tumor volume.
- Spatial descriptors showed that new growth is often not purely boundary-local: mean boundary-growth fraction 0.733, mean distant-growth fraction 0.267, and distant-growth-present rate 0.478. Apparent loss also had a nontrivial core-loss signal: core-loss-present rate 0.279.
- Net-growth transitions were more difficult for LOCF than net-shrinkage transitions: net-growth mean LOCF Dice 0.608 versus net-shrinkage mean LOCF Dice 0.679. Net-growth transitions also had higher relative absolute change (2.045 versus 0.554) and higher distant-growth rate (0.653 versus 0.262).
- Heuristic transition types separated interpretable operating regimes: boundary-growth-dominant cases had high LOCF Dice (0.846, n=16), growth-dominant cases were intermediate (0.625, n=35), loss-dominant cases were intermediate-to-high (0.670, n=30), and mixed growth/loss cases were hardest (0.559, n=53). Persistence-dominant cases were rare in this manifest (n=2) and should not be overinterpreted.
...
## 2026-07-20 - SRD Spatial Transition Taxonomy Result

- Ran the spatial transition-taxonomy analysis on SRD test transitions for fit session 3 and horizons 1, 2, and 3.
- Overall SRD taxonomy: 123 transitions from 41 patients, mean LOCF Dice 0.747, median LOCF Dice 0.852, mean delta 112.5 days.
- SRD is substantially cleaner and more persistence-friendly than SAILOR. Mean relative new growth was 0.363, mean relative apparent loss was 0.126, and mean relative absolute change was 0.489. SAILOR, by comparison, had much larger observed transition burden in the previous taxonomy run: relative new growth 1.092, relative loss 0.285, relative absolute change 1.376.
- Tier behavior separated strongly: Tier A was highly persistence-friendly (mean LOCF 0.902, zero apparent loss, near-zero distant growth); Tier B was difficult due to shrinkage/loss-heavy behavior (mean LOCF 0.590, relative loss 0.290, core-loss rate 0.256); Tier C was growth-heavy with larger tumors and some distant growth (mean LOCF 0.710, relative new growth 0.502, distant-growth rate 0.194).
- Horizon behavior matched intuition: LOCF declined from horizon 1 to 3 (0.869 -> 0.715 -> 0.657), while relative absolute change increased (0.267 -> 0.577 -> 0.624). This supports the horizon/change-burden framing on SRD.
- Net-direction behavior was highly separated: net-growth transitions had high LOCF Dice (0.832, n=96) and almost no loss; net-shrinkage transitions had very low LOCF Dice (0.169, n=18) and high relative loss (0.856); net-stable transitions were perfectly persistent (Dice 1.0, n=9). This is much cleaner than SAILOR, where net-growth/net-shrinkage both include mixed spatial change.
- Transition-type counts showed only three categories under the current rules: growth-dominant (n=62), loss-dominant (n=17), and persistence-dominant (n=44). Mixed growth/loss and distant-growth-present categories did not appear as separate dominant transition types in SRD, unlike SAILOR where mixed growth/loss was common and hardest.
- Interpretation: SRD is useful as a controlled simulation environment for separating growth, shrinkage, and persistence axes, but it does not yet reproduce the full mixed spatial reorganization seen in SAILOR. This is not a fatal flaw; it clarifies the role of SRD as an interpretable controlled space rather than a real-data surrogate. If we want stronger synthetic-real alignment, a future SRD version should intentionally include mixed growth/loss transitions and more distant/offshoot growth behavior.

## 2026-07-20 - Transition Taxonomy Domain-Gap Tooling Added

- Added `scripts/analyze_transition_taxonomy_domain_gap.py` to compare two transition-taxonomy outputs, usually SAILOR as the reference dataset and SRD as the controlled synthetic dataset.
- The script compares numeric transition descriptors, category proportions, transition-type gaps, hard-region coverage, and candidate coverage of reference quantile ranges.
- Motivation: the SRD taxonomy result showed that SRD is cleaner and more axis-separated than SAILOR. Rather than leaving that as a qualitative observation, this analysis quantifies which transition regions SRD covers and which SAILOR-like regions it misses.
- This directly supports the Final Draft Map distinction between SRD as a mechanism-isolation environment versus SRD as a real-data surrogate. The next run should quantify whether SRD lacks mixed growth/loss, high relative absolute change, distant-growth, or core-loss regions relative to SAILOR.

## 2026-07-20 - SAILOR-vs-SRD Transition Domain-Gap Result

- Ran the transition-taxonomy domain-gap analysis using SAILOR spatial taxonomy as the reference and SRD spatial taxonomy as the candidate controlled synthetic dataset.
- The largest numeric gaps were boundary/loss geometry, tumor scale, and non-boundary growth: SAILOR had much higher boundary-loss fraction (0.848 versus 0.171), much larger input/target volumes, and much higher distant-growth fraction (0.267 versus 0.035).
- SAILOR had lower LOCF Dice than SRD (0.639 versus 0.747) and higher observed transition burden: relative new growth 1.092 versus 0.363, relative loss 0.285 versus 0.126, and relative absolute change 1.376 versus 0.489.
- Coverage analysis showed SRD poorly covers several SAILOR reference ranges: for relative loss, 86.2% of SRD transitions were below the SAILOR 10th percentile; for distant-growth fraction, 84.6% were below the SAILOR 10th percentile; for boundary-loss fraction, 82.9% were below the SAILOR 10th percentile. SRD also had no samples above the SAILOR 90th percentile for relative absolute change rate.
- Category gaps were substantial: mixed growth/loss accounted for 39.0% of SAILOR transitions but 0% of SRD transitions; persistence-dominant transitions were 1.5% of SAILOR versus 35.8% of SRD; growth-dominant transitions were 25.7% of SAILOR versus 50.4% of SRD. SAILOR also had more net-shrinkage transitions (44.9% versus 14.6%).
- Hard-region coverage sharpened the mismatch: SAILOR had mixed growth/loss in 53/136 transitions (39.0%), distant-growth fraction >= 0.20 in 65/136 (47.8%), core-loss fraction >= 0.20 in 38/136 (27.9%), and relative absolute change >= 2.0 in 12/136 (8.8%). SRD had 0 mixed growth/loss transitions and 0 transitions with relative absolute change >= 2.0.
- Interpretation: SRD is not a close synthetic surrogate for SAILOR transition complexity. Its value is currently mechanism isolation: it separates clean growth, shrinkage, and persistence axes. SAILOR reveals the harder real-data regime: mixed growth/loss, larger tumor scale, high change burden, non-boundary growth, and apparent loss occur together. A stronger future synthetic generator should include explicit mixed growth/loss transitions, larger scale variability, distant/offshoot growth, and high-change tails if the goal is synthetic-real alignment.

## 2026-07-20 - Forecast-Origin Transition-State Predictability Audit Added

- Added `scripts/analyze_forecast_origin_predictability.py` as the next analysis layer after transition taxonomy and before growth-region ranking.
- Purpose: test whether difficult transition states can be anticipated from information available at the input scan, rather than discovered only after seeing the target scan.
- The script currently supports the following target-side labels as outcomes: mixed growth/loss, distant-growth presence, high transition burden, LOCF breakdown, and high change rate.
- The predictor set is intentionally conservative and forecast-origin only: interval length, input scan span, input tumor volume, current treatment state, treatment change within the input history, previous growth volume, previous loss volume, and previous growth ratio.
- The script explicitly blocks target-derived leakage variables such as target volume, future growth/loss volumes, LOCF Dice, transition type, net direction, target treatment, and spatial outcome fractions from being used as predictors.
...
## 2026-07-20 - SAILOR Forecast-Origin Transition Predictability Result

- Ran `scripts/analyze_forecast_origin_predictability.py` on the SAILOR transition-taxonomy samples using input-length-3, horizon-1 train/validation/test windows.
- Inputs used only forecast-origin predictors: interval length, input span, input tumor volume, current treatment, treatment change within the input history, previous growth/loss volumes, and previous growth ratio.
- Prevalence showed that the target states are clinically/nontrivially present but split-variable: mixed growth/loss was 31.7% in train, 46.4% in validation, and 53.8% in test; distant growth was 47.6%, 53.6%, and 42.3%; LOCF breakdown was sparse, especially on test at 11.5%.
- Logistic models were generally more reliable than shallow trees. Distant-growth presence showed moderate forecast-origin signal: logistic ROC-AUC was 0.723 on validation and 0.776 on test, with average precision 0.781 and 0.769 respectively. Patient-bootstrap balanced-accuracy intervals remained above chance on both splits, though recall/false-negative uncertainty remained wide.
- High transition burden showed weaker but nonzero signal: logistic ROC-AUC was 0.674 on validation and 0.725 on test, with high recall but many false positives. This supports risk-screening more than precise gating.
- LOCF breakdown had apparently strong test discrimination but only 3 positive test cases, so it should be treated as fragile. Recall was high, but false-positive rates were also high, consistent with a conservative warning signal rather than a reliable switch.
...
## 2026-07-25 - SAILOR Forecast-Origin Feature-Ablation Result

- Ran the SAILOR forecast-origin feature ablation across `full_origin`, `no_interval`, `time_only`, `history_only`, and `treatment_only` feature groups.
- `distant_growth_present` showed the cleanest moderate signal. Logistic ROC-AUC stayed useful with `no_interval` features on both validation and test (`0.744` validation, `0.752` test), and patient-bootstrap ROC-AUC intervals stayed mostly above chance. This suggests distant-growth risk is not purely an interval artifact, although history-only performance was unstable across splits.
- `high_change_rate` looked very strong, but the ablation makes it clear that this signal is heavily interval/treatment-linked. `time_only` reached ROC-AUC `0.912` validation and `0.948` test, while `treatment_only` reached `0.924` validation and `0.948` test. This should not be reported as clean biological/history predictability without additional confounding checks.
- `high_transition_burden` showed weak-to-moderate and split-sensitive signal. Full/no-interval/history features were moderately useful on test but less stable on validation. Time-only was strong on test but poor on validation, arguing against a simple interval-only explanation.
- `locf_breakdown` remains fragile because positives are sparse, especially on test. Some test metrics look strong for treatment/time-only features, but validation behavior is weak and the target has too few positive cases for a central claim.
...
## 2026-07-25 - SAILOR Forecast-Origin Confounding Audit Result

- Ran the confounding audit on the SAILOR forecast-origin transition labels using 136 input-length-3, horizon-1 windows from 23 patients.
- Patient concentration check: most labels were spread across multiple patients in the full set, but validation/test subsets remain small and patient-sensitive. `distant_growth_present` had 65 positives across 20/23 patients overall, but test positives were 11 cases across 4 patients with `sub-02` contributing 7/11. `mixed_growth_loss` had 53 positives across 16/23 patients overall, but test positives were heavily concentrated in `sub-02` (10/14). `locf_breakdown` remained sparse with only 27 positives overall and 3 positives on test.
- Treatment distribution was balanced enough across splits to inspect but not enough to ignore: current-treatment rates were 0.671 train, 0.643 validation, and 0.692 test; treatment-change-in-input rates were 0.244, 0.250, and 0.269 respectively.
- Leave-one-patient-out logistic evaluation strengthened the interpretation that some forecast-origin signals are real but uneven. `distant_growth_present` stayed moderately predictable with full-origin ROC-AUC 0.703 and no-interval ROC-AUC 0.703; history-only ROC-AUC was 0.685, while treatment-only dropped to 0.529. This makes distant-growth risk the cleanest current pre-target signal, though patient concentration on small held-out splits still needs caution.
- `high_transition_burden` also survived patient-held-out testing better than expected: full-origin ROC-AUC 0.761, no-interval ROC-AUC 0.736, and history-only ROC-AUC 0.750. Time-only and treatment-only were near chance (0.536 and 0.504), suggesting this label is less reducible to interval/treatment alone.
- `locf_breakdown` remained a secondary, fragile signal. Leave-one-patient-out ROC-AUC was 0.687 for full-origin, 0.716 for no-interval, and 0.730 for history-only, but the positive class is sparse and split-level test support was only 3 positives. This should be framed as exploratory risk scoring, not a reliable switch.
- `mixed_growth_loss` looked more credible under leave-one-patient-out than it did in the earlier fixed test split: full-origin ROC-AUC 0.747, no-interval ROC-AUC 0.753, and history-only ROC-AUC 0.784, while time-only and treatment-only were weak (0.467 and 0.322). However, fixed-split test performance was previously weak and test positives were patient-concentrated, so the current conclusion is "candidate signal requiring repeated patient splits," not a central claim yet.
- `high_change_rate` remained strong under leave-one-patient-out, but the audit confirms it is operationally entangled rather than cleanly biological: full-origin ROC-AUC 0.883, time-only ROC-AUC 0.830, no-interval ROC-AUC 0.816, and treatment-only ROC-AUC 0.739. Because the label is rate-based and treatment/time features carry large signal, this should be described as interval/treatment-linked operating context rather than intrinsic tumor-state predictability.
- Overall interpretation: the forecast-origin audit is useful, but it narrows the claims. The strongest current data-mining result is not "we can predict all hard cases"; it is that different transition states have different pre-target predictability. Distant-growth and high-transition-burden risk appear moderately anticipatable from non-leaking origin features, LOCF breakdown is sparse/fragile, high-change-rate is confounded with interval/treatment structure, and mixed growth/loss needs repeated patient-split validation before being promoted.
- Next robustness step: repeat the forecast-origin ablation/confounding audit over multiple patient splits or leave-one-patient/grouped folds, then report claim status by target label rather than relying on a single train/validation/test assignment.

## 2026-07-25 - Forecast-Origin Patient-Split Stability Tooling Added

- Added `scripts/run_forecast_origin_patient_split_stability.py` to repeat patient-level train/validation/test splits for forecast-origin transition-state predictability.
- The script reuses the same forecast-origin feature groups as the feature-ablation audit: `full_origin`, `no_interval`, `time_only`, `history_only`, and `treatment_only`.
- Each repeat trains only a simple class-balanced logistic classifier. This is intentional: the goal is robustness screening, not to introduce a more flexible model that could overfit small patient splits.
- For every repeat, train-derived quantile labels are recomputed using that repeat's training patients for `high_transition_burden` and `high_change_rate`. This avoids using validation/test distribution information when defining rate/burden thresholds.
- Outputs include per-repeat metrics, target prevalence stability, aggregate metric distributions, a claim-status table, a markdown report, and the exact run configuration.
- The claim-status table is deliberately conservative: labels are marked stable/promising/fragile/weak based on repeated test-split ROC-AUC, balanced accuracy, successful-repeat fraction, and median number of positives.
- Purpose: decide whether `distant_growth_present`, `high_transition_burden`, and `mixed_growth_loss` remain usable forecast-origin risk labels under patient reallocation, or whether they should stay descriptive/exploratory.

## 2026-07-25 - SAILOR Forecast-Origin Patient-Split Stability Result

- Ran the repeated patient-split stability audit on SAILOR using 100 random patient-level train/validation/test splits over 136 input-length-3, horizon-1 windows from 23 patients.
- `mixed_growth_loss` was stronger than the earlier fixed-split result suggested. The best core feature set was `history_only`, with test ROC-AUC mean `0.739`, q25 `0.670`, balanced-accuracy mean `0.709`, q25 `0.636`, recall mean `0.733`, and false-negative-rate mean `0.267`. This upgrades mixed growth/loss from "conflicting/exploratory" to a credible forecast-origin transition-risk signal, though it should still be described as moderate rather than solved.
- `high_transition_burden` remained stable-moderate. Best core feature set was `full_origin`, with test ROC-AUC mean `0.716`, q25 `0.641`, balanced-accuracy mean `0.642`, and median 6 positives per test split. `history_only` and `no_interval` were close, which supports a real origin/history signal rather than a pure interval/treatment artifact.
- `distant_growth_present` stayed promising but split-sensitive. `no_interval` and `full_origin` had similar test ROC-AUC means around `0.675`, but q25 values were near `0.61` and balanced-accuracy q25 was near `0.55`. This remains useful as a moderate risk score, not yet a robust binary gate.
- `locf_breakdown` was better than expected but remains sparse. Best core feature set was `history_only`, with test ROC-AUC mean `0.715`, balanced-accuracy mean `0.701`, and recall mean `0.680`, but median test positives were only 5 and some repeated splits had zero positives. This can support analysis of persistence-risk, but not a central predictive claim by itself.
- `high_change_rate` was the strongest statistical signal: full-origin test ROC-AUC mean `0.905`, q25 `0.854`, balanced-accuracy mean `0.811`, and false-negative-rate mean `0.183`. However, the earlier feature-ablation and confounding checks still apply: this should be framed as an operating-context/rate-risk signal with interval and treatment entanglement, not as clean intrinsic tumor biology.
- Prevalence stability was acceptable for broad transition labels but confirms small-test-split fragility. Median test positives were 11 for distant growth, 9 for mixed growth/loss, 7 for high change rate, 6 for high transition burden, and 5 for LOCF breakdown. Some validation/test repeats had very few positives, especially for LOCF breakdown.
- Main interpretation: repeated patient splits substantially improve our confidence that simple forecast-origin descriptors contain real, moderate signal about upcoming transition complexity. The strongest credible labels for downstream regime-aware analysis are now `mixed_growth_loss`, `high_transition_burden`, and cautiously `distant_growth_present`. `high_change_rate` is operationally useful but conceptually confounded; `locf_breakdown` is useful but sparse.
- Research implication: we now have a defensible bridge from descriptive transition taxonomy to pre-target risk stratification. The next step should not be another generic neural model; it should be to define how these risk signals are used: as evaluation strata, soft sample weights, conditioning variables, or an uncertainty/triage layer for persistence-preserving forecast models.

## 2026-07-25 - Risk-Stratified Model Behavior Tooling Added

- Added `scripts/analyze_risk_stratified_model_behavior.py` to connect forecast-origin risk scores with existing forecasting performance.
- The script generates out-of-fold forecast-origin risk scores using repeated patient-level splits. A sample is scored only when its patient is held out from the risk model for that repeat.
- The script then aggregates risk scores per transition, assigns low/medium/high predicted-risk bins, merges existing per-sample Dice outputs for LOCF and selected models, and summarizes model behavior within each predicted-risk bin.
...
## 2026-07-25 - Risk-Conditioned Forecasting Prototype Tooling Added

- Added `scripts/build_forecast_origin_risk_manifest.py` to create a leakage-controlled risk-augmented longitudinal manifest.
- Training-window risk scores are patient-cross-fitted within the training split. Validation/test risk scores are generated by risk models trained only on the training split. This avoids using a sample's own future transition label as an input to the forecasting model.
- Default risk columns currently use the best-supported transition-risk signals from the recent audits:
  - `risk_mixed_growth_loss_history_only`;
  - `risk_high_transition_burden_full_origin`;
  - `risk_distant_growth_present_no_interval`;
  - `risk_locf_breakdown_history_only`.
